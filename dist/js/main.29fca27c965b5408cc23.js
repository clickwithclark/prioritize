 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */(i,t,n),{user:s}=await e;await yh(),_h({user:s,email:t})}()}catch(e){e.code;const t=e.message;return console.error("An error occurred during sign up: ",t,"Error Code: ",e.code),gl("An error occurred Logging in\n incorrect username or password",6)}}function wh(e){e.preventDefault();(function(e){return H(e).signOut()})(hl()).then((()=>{mh(),yl("logged out successfully")})).catch((e=>{gl("An error occurred logging out")}))}function bh(){Ya(hl(),(e=>{_h(e,!1)})),Dl()}function Ih(){if(!Tl())return gl("You are not connected to the internet\n, try again later...");po(Ve);Ya(hl(),(e=>{if(e)try{return window.confirm("Are you sure you want to overwrite current tasks?")?void yh():gl("Few... !")}catch(e){console.log(e)}return gl("Please sign in to download your tasks !")}))}function Ch(){if(!Tl())return gl("You are not connected to the internet\n, try again later...");Ya(hl(),(e=>{if(!e)return gl("Please register or sign in to save your tasks");{const e=window.confirm("Are you sure you want to overwrite previously saved tasks?");try{if(e){const e=wl()?.todos;if(!e)return;El()}}catch(e){console.log(e)}}}))}function Th(){Ql("keyup","#todoInput",ch),Ql("focusout","#todoInput",lh),Ql("pointerup",".edit",Jl),Ql("pointerup",".drop-down-entry",dh),Ql("pointerdown",".grip",ph),Ql("pointerup","#todoList",uh),Ql("pointerdown",".main-message",hh),Ql("pointerdown",".checkmark",Gl),Ql("pointerdown",".date-sort",Fl.sort),Ql("pointerdown",".category-sort",Ul.sort),Ql("pointerup",".delete-all",zl),Ql("pointerup",".yes-btn",Kl),Ql("pointerup",".no-btn",$l),Ql("pointerup",".delete-selected",Yl),Ql("pointerdown",".tutorial-btn",sh),Ql("pointerdown",".tutorial-ok-btn",rh),Ql("pointerdown","#logo",fh.exportTask),Ql("click","#signup-btn",gh),Ql("click","#logout-btn",wh),Ql("click","#login-btn",vh),window.addEventListener("load",bh),Ql("pointerdown",".download-btn",Ih),Ql("pointerdown",".save-btn",Ch)}"loading"===document.readyState?document.addEventListener("DOMContentLoaded",Th):Th(),"serviceWorker"in navigator&&window.addEventListener("load",(()=>{navigator.serviceWorker.register("./service-worker.js").then((e=>console.log(`Service Worker Registered!😁 \n ${e.scope}`))).catch((e=>console.error(`service worker error:😭${e}`)))}))})()})();